use crate::device::{StoredDevice, DeviceKind};
use std::fmt::Debug;
use thiserror::Error;
...
mod tests {
    ...
    fn test_add_device() {
        ...
        assert!(room.add_device("Thermometer".to_string(), DeviceKind::Thermometer(device)).is_ok());
    }
    ...
    fn test_add_existed_device() {
        ...
        room.add_device("Thermometer".to_string(), DeviceKind::Thermometer(device1))
            .unwrap();

        assert!(room
            .add_device("Thermometer".to_string(), DeviceKind::Thermometer(device2))
            .is_err());
    }
