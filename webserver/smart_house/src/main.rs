use smart_house::{device, edifice, print_report};
...
fn main() {
    ...
    let electrical_outlet_1 = device::DeviceKind::ElectricalOutlet(device::electrical_outlet::ElectricalOutlet {
        state: device::DeviceState::Off,
        power: 0.0,
    });

    let electrical_outlet_2 = device::DeviceKind::ElectricalOutlet(device::electrical_outlet::ElectricalOutlet {
        state: device::DeviceState::Off,
        power: 0.0,
    });
    ...
    let bathroom_thermometer = device::DeviceKind::Thermometer(device::thermometer::Thermometer {
        state: device::DeviceState::Off,
        temperature: 0.0,
    });
